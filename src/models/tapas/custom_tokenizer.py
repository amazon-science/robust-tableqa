...
class CustomTapasTokenizer(PreTrainedTokenizer):
    ...
    def __call__(
        self,
        table: Union[List[pd.DataFrame], "pd.DataFrame"],
        query: Optional[
            Union[
                TextInput,
                PreTokenizedInput,
        ...
        **kwargs
        ...
        if is_batched:
            ...
            assert len(table) == len(queries) and isinstance(table, (list, tuple))
            ...
            if padding == "longest":
                max_ts = (batched_encoding['attention_mask'] == 1).nonzero().numpy()[:, 1].max()
                # print('max_ts', max_ts)
                for key, value in batched_encoding.items():
    ...
    def prepare_for_model(
        ...
        **kwargs
        ...
        column_ranks, inv_column_ranks = self._get_numeric_column_ranks(column_ids, row_ids, raw_table, num_rows=num_rows)
    ...
    def _get_max_num_tokens(self, question_tokens, tokenized_table, num_columns, num_rows, max_length):
        ...
        # This is to disable cell truncation so that the length of returned sequences can be correctly evaluated by ITR.
        ...
        if self.do_extreme_cell_truncation:
            ...
            _, _, current_num_tokens = self._get_table_boundaries(tokenized_table)
            cost = self._get_table_cost(tokenized_table, num_columns, num_rows, current_num_tokens)
            if cost > token_budget:
                return None
            else:
    ...
    def _get_numeric_column_ranks(self, column_ids, row_ids, table, num_rows=None):
